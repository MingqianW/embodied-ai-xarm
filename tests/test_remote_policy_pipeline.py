...
class GripperMappingTests(unittest.TestCase):
    ...
    def test_raw_to_sim_conversion(self) -> None:
        self.assertAlmostEqual(
            gripper_raw_to_ctrl(50.0, self.config), 240.24024, places=4
        )
        self.assertAlmostEqual(
            gripper_raw_to_ctrl(845.0, self.config), 1.501502, places=4
        )

    def test_sim_to_raw_conversion(self) -> None:
        self.assertAlmostEqual(
            menagerie_ctrl_to_raw_target(240.24024, self.config), 50.0, places=3
        )
        self.assertAlmostEqual(
            menagerie_ctrl_to_raw_target(1.501502, self.config), 845.0, places=3
        )
