...
def test_force_override_scales_both_position_servo_terms() -> None:
    ...
    assert model.actuator_gainprm[actuator_id, 0] == pytest.approx(1000.0)
    assert model.actuator_biasprm[actuator_id, 1] == pytest.approx(-1000.0)
    np.testing.assert_allclose(
        model.actuator_forcerange[actuator_id], baseline_forcerange
    )
    assert values["baseline"]["actuator"]["gainprm"][0] == pytest.approx(500.0)
    assert values["effective"]["actuator"]["gainprm"][0] == pytest.approx(1000.0)
        ...
        rows.append(
            {
                ...
                "command": {
                    ...
                    "gripper_ctrl": 1.5 if index >= 15 else 105.0,
                },
                "contacts": {
...
def test_three_patch_geometry_variant_compiles_and_preserves_pad_mass() -> None:
    setting = next(
        row
    ...
    for side in ("left", "right"):
        names = [
            f"{side}_fingertip_pad",
            f"{side}_fingertip_pad_lower",
            f"{side}_fingertip_pad_upper",
        ]
        geom_ids = [
            mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, name) for name in names
        ]
        assert all(geom_id >= 0 for geom_id in geom_ids)
        body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, f"{side}_finger")
        assert model.body_mass[body_id] == pytest.approx(0.06)
