...
def _menagerie_baseline(root: Path | None) -> dict[str, Any] | None:
    if root is None:
        return None
    root = root.expanduser().resolve()
    open_close_path = root / "open_close/open_close_validation.json"
    grasp_path = root / "grasp_slip/analysis_v2/summary.json"
    if not open_close_path.is_file() or not grasp_path.is_file():
        raise FileNotFoundError(f"Incomplete Menagerie validation root: {root}")
    open_close = json.loads(open_close_path.read_text(encoding="utf-8"))
    trials = json.loads(grasp_path.read_text(encoding="utf-8"))
    raw_sweep = open_close["raw_sweep"]
    suspended = [row for row in trials if row["hold_kind"] == "suspended"]
    oracle = [row for row in suspended if row["command_variant"] == "oracle_command"]

    def optional_mm(value: Any) -> float | None:
        return None if value is None else 1000.0 * float(value)

    return {
        "root": str(root),
        "scope": "current untouched Menagerie baseline; existing diagnostic traces only",
        "dynamic_sweep": {
            "count": len(raw_sweep),
            "strictly_monotonic": bool(open_close["measured_direction"]),
            "maximum_driver_symmetry_error_rad": open_close[
                "maximum_driver_symmetry_error_rad"
            ],
            "raw_round_trip_error": _quantiles(
                [row["raw_round_trip_error"] for row in raw_sweep]
            ),
            "rows": raw_sweep,
        },
        "suspended_oracle": {
            "trial_count": len(oracle),
            "mechanically_stable_2mm_count": int(
                sum(bool(row["mechanically_stable_2mm"]) for row in oracle)
            ),
            "contact_loss_count": int(
                sum(row["sustained_contact_loss_s"] is not None for row in oracle)
            ),
            "maximum_downward_slip_mm": _quantiles(
                [1000.0 * row["maximum_downward_slip_m"] for row in oracle]
            ),
            "by_task": {
                row["task"]: {
                    "maximum_downward_slip_mm": 1000.0 * row["maximum_downward_slip_m"],
                    "mechanically_stable_2mm": bool(row["mechanically_stable_2mm"]),
                    "hold_bilateral_contact_fraction": row[
                        "hold_bilateral_contact_fraction"
                    ],
                    "mean_bilateral_normal_force_sum_n": row[
                        "mean_bilateral_normal_force_sum_n"
                    ],
                    "maximum_target_penetration_mm": optional_mm(
                        row["maximum_target_penetration_m"]
                    ),
                }
                for row in oracle
            },
...
def main() -> None:
    ...
    if not os.environ.get("SLURM_JOB_ID"):
        raise RuntimeError("Full dataset audit must run inside a Slurm job")
    ...
    result = {
        ...
        "current_menagerie_baseline": _menagerie_baseline(
            args.menagerie_validation_root
        ),
        "identifiability": {
            "directly_observable": [
        ...
        "limitations": [
            ...
            "The authoritative simulation training dataset predates the Menagerie integration and is selection-filtered for successful oracle episodes.",
            "Current Menagerie evidence comes from the existing baseline diagnostic suite, not a regenerated IL dataset.",
        ],
    }
