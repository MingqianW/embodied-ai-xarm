...
def _model_configuration(
    ...
    *,
    ...
    equality_name = "driver_symmetry" if menagerie else "symmetric_gripper"
    ...
    return {
        ...
        "actuator": {
            ...
            "representation": "menagerie_affine" if menagerie else "legacy_position",
...
def _apply_overrides(
    ...
    *,
    ...
    if force_limit is None:
        ...
        if not np.allclose(
            model.actuator_forcerange[actuator_id], [-50.0, 50.0], atol=1e-12
        ):
            raise RuntimeError(
                "Menagerie force-range sweep requires untouched [-50, 50] baseline"
            )
        model.actuator_forcerange[actuator_id] = [-force_limit, force_limit]
    kp_multiplier = float(setting.get("kp_multiplier", 1.0))
    # Diagnostic legacy suites scale affine gain and length bias together so
    # the unloaded equilibrium is unchanged. The Menagerie baseline uses 1x.
    ...
    if setting.get("condition") is not None:
        if baseline["simulation"]["cone"] != "pyramidal" or not math.isclose(
            float(baseline["simulation"]["impratio"]), 1.0
        ):
            raise RuntimeError(
                "Validated baseline is not pyramidal/impratio=1: "
