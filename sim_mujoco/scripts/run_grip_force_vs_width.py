...
GRIPPER_EQUALITY = "driver_symmetry"
...
def build_runtime_model_xml(*, gripper_force_limit: float = 50.0) -> str:
    ...
    actuator = root.find(f".//actuator/general[@name='{GRIPPER_ACTUATOR}']")
    if actuator is None:
...
def _position_fixture(
    ...
    errors.extend(
        [
            ...
            max(float(np.max(np.abs(value - frame))) for value in pad_frames[1:]),
        ]
    )
    return {
        "width_mm": width_mm,
...
def _sample(
    ...
    *,
    ...
    moment = np.asarray(data.actuator_moment[actuator], dtype=np.float64)
    pad_centers = [
...
def validate_runtime_model(*, gripper_force_limit: float = 50.0) -> dict[str, Any]:
    ...
    errors: list[str] = []
    if simulation["cone"] != int(mujoco.mjtCone.mjCONE_PYRAMIDAL):
        errors.append("cone is not pyramidal")
    if not math.isclose(simulation["impratio"], 1.0, abs_tol=1e-12):
        errors.append("impratio is not 1")
    if not math.isclose(simulation["timestep_s"], 0.002, abs_tol=1e-12):
        errors.append("timestep is not 0.002 s")
    if not math.isclose(actuator["gainprm"][0], 0.333, abs_tol=1e-12):
        errors.append("Menagerie actuator gain is not 0.333")
    if not np.allclose(actuator["biasprm"][:3], [0.0, -100.0, -10.0], atol=1e-12):
        errors.append("Menagerie affine actuator bias changed")
    if actuator["ctrlrange"] != [0.0, 255.0]:
