"""Project-raw conversions for the Menagerie UFACTORY xArm Gripper.
...
Menagerie's actuator uses 0 open through 255 closed and drives the fixed
``split`` tendon with an affine force law.
"""
...
FINGER_INNER_SURFACE_OFFSET_M = 0.026003
...
def raw_gripper_to_sim_slide(
    ...
    """Map project raw state to the retired simplified slide coordinate.

    This remains available only so runtime diagnostics can reproduce the
    frozen pre-Menagerie split-pad baseline without modifying production MJCF.
    """
...
def sim_slide_to_raw_gripper(
    ...
    """Invert :func:`raw_gripper_to_sim_slide` for legacy diagnostics."""
...
def raw_gripper_to_menagerie_ctrl(
    ...
    *,
    ...
    return driver_angle_to_menagerie_ctrl(
        raw_gripper_to_driver_angle(
            raw_value,
            config_or_mapping,
            operational_bounds=operational_bounds,
        )
...
def set_menagerie_gripper_configuration(
    ...
    *,
    ...
    """Set all six linkage hinges to the matching URDF mimic configuration."""
...
def measure_fingertip_aperture_m(
    ...
    if base_id < 0:
        raise RuntimeError("xarm_gripper_base_link body not found")
