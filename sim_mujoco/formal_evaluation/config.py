...
def load_protocol(path: Path | None = None) -> FormalProtocol:
    ...
    protocol = FormalProtocol(
        ...
        camera_config_path=_absolute_path(raw["paths"]["camera_config"], config_path=path),
        task_scene_config_path=_absolute_path(raw["paths"]["task_scene_config"], config_path=path),
        robot_xml_path=_absolute_path(raw["paths"]["robot_xml"], config_path=path),
...
def validate_protocol(protocol: FormalProtocol) -> None:
    ...
    if not str(protocol.output_root).startswith("/work/nvme/bfmk/mw89/"):
