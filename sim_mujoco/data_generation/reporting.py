...
1. The legacy `sim_mujoco/scripts/collect_oracle_data.py` collector was a red-block-only canonical collector.
2. `sim_mujoco/scripts/collect_real_raw_sim_data.py` was the active six-task collector, but its plan and prompts were hard-coded and included distractor splits.
...
10. The established six-task conversion path used the real-compatible raw CSV/PNG layout and the shared `fine_tune/xarm_lerobot_writer.py` LeRobot v2.1 writer.
...
- Joint real/simulation training must normalize historical real aliases through `sim_mujoco.data_generation.registry`; old datasets are not mutated.
...
```bash
cd {repository}
{config.path.parents[3]}/slurm/simulation_data/offline_tests.sbatch  # submit with sbatch
