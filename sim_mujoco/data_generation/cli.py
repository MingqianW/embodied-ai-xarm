...
from sim_mujoco.data_generation.audit import (
    audit_converted,
    audit_raw,
    write_audit_reports,
...
from sim_mujoco.data_generation.collection import collect
from sim_mujoco.data_generation.config import load_pipeline_config
from sim_mujoco.data_generation.conversion import convert_dataset
from sim_mujoco.data_generation.manifest import atomic_write_json
from sim_mujoco.data_generation.reporting import (
    write_final_handoff,
    write_initial_audit,
)
from sim_mujoco.data_generation.safety import (
    apply_group_permissions,
    path_inventory,
    replace_authorized_roots,
    validate_authorized_root,
)
from sim_mujoco.data_generation.status import git_sha, write_status


DEFAULT_CONFIG = Path("sim_mujoco/config/data_generation/clean_multitask_stable_v3.yaml")
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="python -m sim_mujoco.data_generation.cli")
    ...
    if args.command == "generate":
        ...
        if args.initialize_log_root:
            result = replace_authorized_roots(
                ...
                git_sha=git_sha(config.path.parents[3]), config_path=config.path,
            )
