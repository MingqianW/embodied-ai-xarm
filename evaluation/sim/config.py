...
def _task_specs(rows: list[dict[str, Any]]) -> tuple[EvaluationTask, ...]:
    expected = tuple(EvaluationTask(task.task_id, task.prompt) for task in TASKS)
    actual = tuple(
        EvaluationTask(str(row["task_id"]), str(row["prompt"])) for row in rows
    )
    if actual != expected:
        raise ValueError(
