...
def validate_episode_result(result: dict[str, Any]) -> None:
    ...
    if result.get("backend", "sim") != "sim":
        raise ValueError("Formal simulation episode result backend must be 'sim'")
    ...
    if provenance.get("backend", "sim") != "sim":
        raise ValueError("Formal simulation provenance backend must be 'sim'")
    for key in (
        "protocol_sha256",
        "model_spec_sha256",
        "openpi_git_commit",
