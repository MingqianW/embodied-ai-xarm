...
from data.common.records import EpisodeRecord
...
def write_xarm_lerobot_dataset(
    records_by_episode: list[list[dict[str, Any]] | EpisodeRecord],
    *,
    repo_id: str,
    ...
    written_frames = 0
    try:
        for episode_value in records_by_episode:
            episode_records = (
                episode_value.as_writer_records()
                if isinstance(episode_value, EpisodeRecord)
                else episode_value
            )
            if not episode_records:
                raise ValueError("Cannot write an empty LeRobot episode")
            for record in episode_records:
                training_record = validate_training_record(record)
                state = training_record["state"]
                actions = training_record["actions"]
                task = training_record["task"]
                frame = {
                    "image": load_rgb(training_record["image"]),
                    "wrist_image": load_rgb(training_record["wrist_image"]),
                    "state": state,
                    "actions": actions,
                }
